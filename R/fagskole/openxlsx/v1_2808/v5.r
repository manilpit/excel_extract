library(openxlsx)
############### FUNKSJONSDEFINISJON OG OPPSETT ###############
...
############### INNLESING AV FILER OG VALIDERING ###############
...
############### HOVEDLØKKE FOR FILBEHANDLING ###############
  for (fil_nr in seq_along(excel_filer)) {
    fil <- excel_filer[fil_nr]
    cat(paste("\nBehandler fil:", basename(fil), "\n"))
    
    ############### INNLESING AV KILDEDATA ###############
    # Les data fra arkene og legg til debugging
    resultat_data <- read.xlsx(fil, sheet = "Resultatregnskap")
    print("Første 10 rader i Resultatregnskap:")
    print(head(resultat_data, 10))
    
    eiendeler_data <- read.xlsx(fil, sheet = "Balanse - eiendeler")
    gjeld_ek_data <- read.xlsx(fil, sheet = "Balanse - egenkapital og gjeld")
    
  ############### IDENTIFISERING AV FAGSKOLE ###############
    # Hent fagskolenavn fra kolonnenavnet
    kolonne_navn <- names(resultat_data)[1]  # Får første kolonnenavn
    if (!grepl("^Fagskolens\\.navn\\:", kolonne_navn)) {
      cat("ADVARSEL: Fant ikke fagskolenavn i", basename(fil), ". Hopper over denne filen.\n")
      next
    }
    
    # Hent ut navnet fra kolonnenavnet
    fagskole_navn_fra_fil <- sub("^Fagskolens\\.navn\\:\\.", "", kolonne_navn)
    fagskole_navn_fra_fil <- trimws(toupper(fagskole_navn_fra_fil))
    
    cat("Fant fagskolenavn:", fagskole_navn_fra_fil, "\n")
