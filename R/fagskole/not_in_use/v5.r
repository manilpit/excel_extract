...
# Ny funksjon: Hent fagskole-ID fra filnavn
...
# Ny funksjon: Hardkodet mapping mellom fagskolenummer og rad i målfilen
...
excel_overforing_fagskole <- function(
  ...
  # Sjekk om vi har hardkodede rader for denne fagskole-ID
  target_rows <- get_target_rows(fagskole_id)
  
  if (!is.null(target_rows)) {
    # Bruk hardkodede rader om de er tilgjengelige
    ...
    # Finn raden i målarket hvis vi ikke har hardkodede verdier
    if (is.null(resultat_rad)) {
