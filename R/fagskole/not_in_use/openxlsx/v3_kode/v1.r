...
find_row_by_code <- function(data, code) {
  ...
  for (row in 1:nrow(data)) {
    cell_value <- normalize(as.character(data[row, 5]))
    if (cell_value == code) {
...
# Funksjon for å finne flere rader for summering (omløpsmidler)
find_multiple_rows_by_codes <- function(data, codes) {
  rows <- c()
  for (code in codes) {
    row <- find_row_by_code(data, code)
    if (!is.na(row)) {
      rows <- c(rows, row)
    }
  }
