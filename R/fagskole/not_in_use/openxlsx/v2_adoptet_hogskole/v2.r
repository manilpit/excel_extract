...
match_institution_row <- function(wb_path, sheet, inst_name, start_row = 6, name_col = 1) {
  ...
  # Strip away "Fagskolens navn:" if it's still there
  target <- sub("^FAGSKOLENS NAVN:\\s*", "", target)
  ...
  # Hvis navnet inneholder et tall, prøv å matche på bare tallet
  if (grepl("\\d+", target)) {
    num <- regmatches(target, regexpr("\\d+", target))
    if (length(num) > 0) {
      for (i in seq_along(names_vec)) {
        if (grepl(paste0("\\b", num, "\\b"), names_vec[i])) {
          cat("Matchet på nummer:", num, "->", tab[start_row + i - 1, name_col], "\n")
          return(start_row + i - 1)
        }
      }
    }
...
# Forbedret funksjon for å hente fagskolens navn fra kilde
get_school_name_from_source <- function(src_df) {
  # Sjekk etter "Fagskolens navn:" i de første radene
  for (r in 1:min(10, nrow(src_df))) {
    for (c in 1:min(5, ncol(src_df))) {
      cell_value <- as.character(src_df[r, c])
      # Gjør om til upper case for å fange opp ulike variasjoner
      cell_upper <- toupper(cell_value)
      
      if (grepl("FAGSKOLENS NAVN:", cell_upper)) {
        # Hent ut teksten etter kolonet
        school_name <- sub(".*FAGSKOLENS NAVN:\\s*", "", cell_value, ignore.case = TRUE)
        school_name <- trimws(school_name)
        
        if (nchar(school_name) > 0) {
          cat("Fant skolenavn:", school_name, "\n")
          return(school_name)
        }
      }
    }
  ...
  # Sjekk også filnavn-mønster (NNN_YYYY_Regnskap_XXXXXX.xlsx)
  first_cell <- as.character(src_df[1, 1])
  if (grepl("^\\d+_\\d{4}_Regnskap_\\d+", first_cell)) {
    school_id <- sub("^(\\d+)_.*", "\\1", first_cell)
    return(paste("Fagskole", school_id))
  }
  
  # Hvis alt annet feiler, returner første celle
  warning("Kunne ikke finne spesifikt skolenavn, bruker første celle: ", substr(as.character(src_df[1, 1]), 1, 30), "...")
  return(as.character(src_df[1, 1]))
}
# ------- Hovedfunksjon -------
