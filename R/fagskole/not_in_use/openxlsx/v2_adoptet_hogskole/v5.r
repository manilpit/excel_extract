...
# Ny funksjon for å finne kolonner basert på headers
find_col_by_header <- function(headers, pattern) {
  matches <- which(grepl(pattern, headers, perl = TRUE))
  if (length(matches) > 0) {
    cat("\nFant kolonne for", pattern, ":", matches[1], "\n")
    return(matches[1])
  }
  cat("\nFant ikke kolonne for:", pattern, "\n")
  return(NA_integer_)
}
...
find_source_rows <- function(src_df) {
  find_row <- function(pat) {
    for (col in 1:min(3, ncol(src_df))) {  # Sjekk de første kolonnene
      ...
      if (length(i) > 0) {
        cat("\nFant rad", pat, ":", i[1], "i kolonne", col, "\n")
        return(i[1])
      }
    }
...
excel_overforing_fagskole <- function(
  ...
  for(i in seq_along(res_headers)) {
    cat("Kolonne", i, ":", res_headers[i], "\n")
  }
  cat("\nFunnet headers i balanse-ark:\n")
  for(i in seq_along(bal_headers)) {
    cat("Kolonne", i, ":", bal_headers[i], "\n")
  }

  # Finn kolonner basert på headers
  c_res_dm_2024 <- find_col_by_header(res_headers, "DRIFTSMARGIN.*2024")
  c_res_dm_2023 <- find_col_by_header(res_headers, "DRIFTSMARGIN.*2023")
  c_res_di_2024 <- find_col_by_header(res_headers, "DRIFTSINNTEKTER.*2024")
  c_res_di_2023 <- find_col_by_header(res_headers, "DRIFTSINNTEKTER.*2023")

  c_bal_om_2024  <- find_col_by_header(bal_headers, "OML[ØO]PSMIDLER.*2024")
  c_bal_om_2023  <- find_col_by_header(bal_headers, "OML[ØO]PSMIDLER.*2023")
  c_bal_ek_2024  <- find_col_by_header(bal_headers, "EGENKAPITAL(?!GRAD).*2024")
  c_bal_ek_2023  <- find_col_by_header(bal_headers, "EGENKAPITAL(?!GRAD).*2023")
  c_bal_kg_2024  <- find_col_by_header(bal_headers, "KORTSIKTIG.*GJELD.*2024")
  c_bal_kg_2023  <- find_col_by_header(bal_headers, "KORTSIKTIG.*GJELD.*2023")
  c_bal_tot_2024 <- find_col_by_header(bal_headers, "(TOTALKAPITAL|TOTALE?\\s+EIENDELER|SUM\\s+EIENDELER).*2024")
  c_bal_tot_2023 <- find_col_by_header(bal_headers, "(TOTALKAPITAL|TOTALE?\\s+EIENDELER|SUM\\s+EIENDELER).*2023")
  c_bal_ekg_2024 <- find_col_by_header(bal_headers, "EGENKAPITALGRAD.*2024")
  c_bal_ekg_2023 <- find_col_by_header(bal_headers, "EGENKAPITALGRAD.*2023")
  c_bal_f2_2024  <- find_col_by_header(bal_headers, "(FINANSIERINGSGRAD\\s*2|LIKVIDITETSGRAD).*2024")
  c_bal_f2_2023  <- find_col_by_header(bal_headers, "(FINANSIERINGSGRAD\\s*2|LIKVIDITETSGRAD).*2023")
  ...
  safe_write <- function(sheet, value, row, col, description) {
    if (!is.na(col) && !is.na(value)) {
      ...
      cat("Hopper over", description, "- manglende kolonne eller verdi\n")
    }
  }
